...
function evalExp(exp,envir) {
	...
	return E.createClo(A.getFnExpParams(exp),
		...
		var whileSentinel = A.getWhileExpParams(exp);
		var whileBody = A.getWhileExpBody(exp);
		var sentinelEval = evalExp(whileSentinel,envir)
		if(E.getBoolValue(sentinelEval) === true){
			evalExps(whileBody, envir);
			evalExp(exp, envir);
		}
		return undefined;
	}
	...
	}else if(exp.comesFromLetRecBlock){
		var dummyClo =  E.createClo([], [], []);
		var fArgs = A.getAppExpArgs(exp);
		var currentClosure = E.getEnvEnv(envir);
		dummyClo[0] = fArgs;
		var f = A.getAppExpFn(exp);
		var updatedEnvir = E.update(envir, f, dummyClo);
		return callByValue(exp, updatedEnvir);
	}else {
	    switch (ppm) {

...
public class off_file_object 
{
    ...
    float[] spherical_map(int n){
        float[] texture_coords = new float[2];
        
        // Convert (x,y,z) point to spherical coords.
        float r = (float)Math.sqrt(Math.pow(vertices[n][0],2) + Math.pow(vertices[n][1],2) + Math.pow(vertices[n][2],2));
        float theta = (float)Math.atan(vertices[n][1]/vertices[n][0]);
        float phi = (float)Math.acos(vertices[n][2] / r);
        
        // Convert spherical coords to [0,1] texture space
        texture_coords[0] = (theta + (float)Math.PI/2.0f) / ((float)Math.PI);
        texture_coords[1] = (phi) / ((float)Math.PI);
